...
public class CreateStoryActivity extends StoryActivityBase {
	...
	private void launchSoundIntent() {
		...
		Uri recordFilename = getOutputMediaFileUri(MEDIA_TYPE_AUDIO);
		
		// Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the SoundRecordActivity class, EXTRA_OUTPUT
		soundRecordIntent.putExtra(SoundRecordActivity.EXTRA_OUTPUT, recordFilename.getPath());
	...
	private void launchVideoCameraIntent() {
		...
		// Hint: use standard Intent from MediaStore class
		// See: http://developer.android.com/reference/android/provider/MediaStore.html
		Intent videoCamaraIntent = new Intent(MediaStore.ACTION_VIDEO_CAPTURE);
		...
		Uri filePath = getOutputMediaFileUri(MEDIA_TYPE_VIDEO);

		
		//  Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		videoCamaraIntent.putExtra(MediaStore.EXTRA_OUTPUT, filePath);
		...
		videoCamaraIntent.putExtra(MediaStore.EXTRA_VIDEO_QUALITY, 1);

		
		//  Start a new activity for result, using the new intent and the request
		// code CAMERA_VIDEO_REQUEST
		startActivityForResult(videoCamaraIntent, CAMERA_VIDEO_REQUEST);	
	}
